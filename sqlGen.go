...
type SQLVar struct {
	...
	CurrentTable      string   `xml:"CurrentTable"`
...
type SQLAction struct {
	...
	MapField       string    `xml:"mapField,attr"`
...
func getIdx(cTable string) (idxMap map[string]SQLVar) {
	...
	for rows.Next() {
		...
		if tSV, ok := tmpMap[name1]; !ok {
			...
			pkV.IdxName = name1
			pkV.IdxCols = strings.ToLower(name3)
			tmpMap[name1] = pkV
		} else {
			tSV.IdxCols += ","
			tSV.IdxCols += strings.ToLower(name3)
			tmpMap[name1] = tSV
		}
	}
	rows.Close()
...
func BuildInData() {
	...
	for a, inPath := range gSQLModelConf.InPath {
		...
		for _, f := range fileList {
			...
			cTable, HisTable := strings.ToUpper(strings.TrimSpace(cSlice[2])), strings.ToUpper(strings.TrimSpace(cSlice[2]))
